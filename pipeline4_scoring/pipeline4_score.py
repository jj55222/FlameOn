...
DEFAULT_PASS2_MODEL = os.environ.get("P4_PASS2_MODEL", "qwen/qwen3.6-plus:free")
...
def run_pass1(merged, backend):
    ...
    try:
        ...
        raise LLMError(f"Pass 1 JSON parse failed: {e}\nFirst 500 chars: {raw[:500]}")
...
def run_pass2(merged, pass1, weights, backend, scoring_breakdown, narrative_score):
    ...
    try:
        ...
        raise LLMError(f"Pass 2 JSON parse failed: {e}\nFirst 500 chars: {raw[:500]}")
...
def score_case(
    ...
    print(f"  [Scoring] narrative_score={scoring['narrative_score']:.1f} "
          f"(density={scoring['scoring_breakdown']['moment_density_score']:.1f}, "
          f"arc={scoring['scoring_breakdown']['arc_similarity_score']:.1f}, "
          f"artifact={scoring['scoring_breakdown']['artifact_completeness_score']:.1f}, "
          f"unique={scoring['scoring_breakdown']['uniqueness_score']:.1f})")
...
def append_batch_summary(verdict, output_dir):
    ...
    with open(summary_path, "a", encoding="utf-8") as f:
        ...
        f.write(
            ...
            f"{bd.get('moment_density_score', 0):.1f}\t{bd.get('arc_similarity_score', 0):.1f}\t"
            f"{bd.get('artifact_completeness_score', 0):.1f}\t{bd.get('uniqueness_score', 0):.1f}\t"
